...
class SAGE(nn.Module):
    def __init__(self,
        ...
        self.num_layers = num_layers
        self.gat_layers = nn.ModuleList()
        self.gat_layers.append(SAGEConv(in_dim,num_hidden,aggregator_type = 'pool'))
        for j in range(1,num_layers):
            self.gat_layers.append(SAGEConv(num_hidden,num_hidden,aggregator_type = 'pool'))
        self.gat_layers.append(SAGEConv(num_hidden,num_classes,aggregator_type = 'pool'))
    def forward(self, inputs, middle=False):
        h = inputs
        middle_feats = []
        for l in range(self.num_layers):
